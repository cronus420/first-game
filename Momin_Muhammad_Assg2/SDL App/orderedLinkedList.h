...
{
public:
    bool search(const Type& findItem) const;
      //Function to determine whether findItem is in the list.
      //Postcondition: Returns true if findItem is in the list,
...
template <class Type>
bool orderedLinkedList<Type>::search(const Type& findItem) const
{
    bool found = false;
    ...
    if (found)
       found = (current->info == findItem); //test for equality
...
void orderedLinkedList<Type>::deleteNode(const Type& deleteItem)
{
    ...
    else
    {
        ...
        if (current == NULL)   //Case 4
            cout << "The items to be deleted is not in the "
                 << "list." << endl;
        else
